...
%Tables
%charts
\section{Building applications based on the DDE framework}
\label{sec:result-framework}
  Two sample applications that were the created based on the DDE framework are discussed in Case Study~[\autoref{chapter:case_study}]. The sample applications \autoref{sec:producer-consumer-program} and
  \autoref{sec:request-reply-program} were used for testing and evaluation of the framework. The benchmarks discussed in \autoref{sec:benchmarks} are the results of this evaluation.

  Some observations made during the development and the deployment of applications based on the DDE framework were:

\begin{itemize}
  \item The program source codes based on the DDE frameworks are short. It is evident from the source codes of the applications used in case study~\autoref{chapter:case_study}.
